...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost = (gpt4o_tokens / 1000) * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Keeping last 6 messages ensures we keep the last 3 full turns (user+assistant)
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        p = trunc(r.get('prompt', ''))
        g4 = trunc(r.get('gpt4o_response', ''))
        gm = trunc(r.get('mini_response', ''))
        l4 = f"{r.get('gpt4o_latency', 0):.2f}"
        lm = f"{r.get('mini_latency', 0):.2f}"
        lines.append(f"{p:<40} | {g4:<40} | {gm:<40} | {l4:<15} | {lm:<15}")
